...
type LibName = 'typescript' | '@types/node' | "xstate"
...
export class TSFS {
    ...
    public forFileInLib = async (
        ...
        const meta = await import(`~/types/${libName}/meta.js`)
        ...
        if (isCached) {
            ...
            // Rollup needs us to use static strings for dynamic imports
            const data = await import(`~/types/${libName}/data.js`  )
    ...
    injectCoreLibs = async () => {
        await Promise.all([
            ...
            // this.forFileInLib('xstate', (name, content) => {
            //    
            //      this.fs.set('/' +name, content)
            // }),
        ])
