...
const server:Server = Server.configure({
    port: 4567,
    address: '127.0.0.1',
    name: 'default',
  extensions: [
...
export function hocuspocusHandler(request: Request, info:Deno.ServeHandlerInfo<Deno.NetAddr>) { 
  
    // const response = new Response()
    const {socket, response} = Deno.upgradeWebSocket(request);
    return response;

    // server.requestHandler(request, new Response())
   //  const socket = new WebSocket({
   //      request,
   //      server: server.webSocketServer
   //  }); 
   // return server.handleConnection(socket,  request)  
}
