...
import fakeCatalog, { type Product }  from './catalog';
export type {Product} from './catalog';
 
export function connectStore (doc: Y.Doc, key?: string, batch=8, category: Product["category"] = "shoes") {

    const page = doc.getArray(key || 'catalog');
    const source = doc.getArray('availableItems');
    source.push(fakeCatalog[category]);
