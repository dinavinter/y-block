...
const YElement = c(function({ element }) {
  const [attributes, setAttributes] = useState({});
  const [children, setChildren] = useStateArray([]);
  ...
  const ChildTemplates = useProxySlot(refSlotTemplate, (el) => el instanceof HTMLElement);
  useEffect(() => {
    if (!element) return;
    console.debug("YElement:useEffect", { element: element.toJSON() });
    setAttributes(element.getAttributes());
    setChildren(element.toArray());
    element.observe((event) => {
      console.log("YElement:observe", { event, element: element.toJSON() });
      setAttributes(element.getAttributes());
      setChildren(element.toArray());
    });
  }, [element]);
  function getSharedProps(Template, attributes2) {
    console.log("YElement:getSharedProps", { Template, attributes: attributes2 });
    return Template.getAttributeNames().filter((name) => name.startsWith(":")).reduce((acc, name) => {
      ...
      acc[propName] = attributes2[attrValue || propName];
      return acc;
    }, {});
  }
  return /* @__PURE__ */ jsxs("host", { shadowDom: true, children: [
    /* @__PURE__ */ jsx("template", { children: /* @__PURE__ */ jsx("slot", { ref: refSlotTemplate }) }),
    /* @__PURE__ */ jsx("template", { children: /* @__PURE__ */ jsx("slot", { ref: refChildTemplate, name: "each" }) }),
    Templates.map(
    ...
    children.map(
      (child, index) => ChildTemplates.map(
        (Template) => /* @__PURE__ */ jsx(Template, { cloneNode: true, ...getSharedProps(Template, {
          element,
          text: element,
          hook: element,
